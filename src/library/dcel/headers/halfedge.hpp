...
public:
  ...
  /**
   * \fn Halfedge( Vertex* vertex , Edge* edge , Face* face , Halfedge* next ,
   * Halfedge* prev )
   *
   ...
   * cycle containing this halfedge.
   */
  Halfedge(Vertex *vertex, Edge *edge, Face *face, Halfedge *next,
           Halfedge *prev) {
    set_origin(vertex);
    set_edge(edge);
    set_face(face);
   ...
   */
  Vertex *get_origin() const { return _vertex; }
   ...
   * half-edge.
   */
  void set_origin(Vertex *vertex) { _vertex = vertex; }
   ...
   * half-edge belongs to.
   */
  Face *get_face() const { return _face; }
   ...
   */
  Halfedge *get_next() const { return _next; }
   ...
   * half-edge cycle this half-edge belongs to.
   */
  void set_next(Halfedge *next) { _next = next; }
   ...
   * cycle this half-edge belongs to.
   */
  Halfedge *get_prev() const { return _prev; }
   ...
   * half-edge cycle this half-edge belongs to.
   */
  void set_prev(Halfedge *prev) { _prev = prev; }
   ...
   * halfedge.
   */
  HAttrib &get_attributes() { return _attributes; }
...
private:
  ...
  Vertex *_vertex =
      nullptr;           ///< Pointer to the origin vertex of this half-edge.
  Edge *_edge = nullptr; ///< Pointer to the edge this half-edge belongs to.
  Face *_face = nullptr; ///< Pointer to the face whose half-edge cycle this
                         ///< half-edge belongs to.
  Halfedge *_next = nullptr; ///< Pointer to the next half-edge in the cycle
                             ///< this half-edge belongs to.
  Halfedge *_prev = nullptr; ///< Pointer to the previous half-edge in the cycle
                             ///< this half-edge belongs to.
