 ...
 * (i.e., a point  in \f$R^3\f$) from a surface  mesh represented by a
 * DCEL data structure.
 *
 * \author
 ...
 * \class Vertex
 *
 * \brief  This  class  represents   a  vertex  (i.e.,  a  point  in
 * \f$R^3\f$) from  a triangle surface mesh represented  by the DCEL
 * data structure.
 */
template <typename VAttrib, typename FAttrib, typename EAttrib,
          typename HAttrib>
...
public:
   ...
   */
  void set_x_coord(double x) { _x = x; }
   ...
   * coordinate of the location to this vertex.
   */
  void set_y_coord(double y) { _y = y; }
   ...
   * coordinate of the location to this vertex.
   */
  void set_z_coord(double z) { _z = z; }
   ...
   * vertex.
   */
  VAttrib &get_attributes() { return _attributes; }
...
private:
  ...
  Halfedge *_halfedge =
      nullptr;         ///< Pointer to a half-edge with origin at this vertex.
