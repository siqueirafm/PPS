 ...
 * \file ppsfrompnt.hpp
 *
 * \brief  Definition  of  the  class  PPSfromPNT  that  represents  a
 ...
 * \namespace ppsfrompnt
 *
 * \brief The namespace ppsfrompnt contains all classes that implement
 * a Parametric  Pseudo-Surface (PPS) that approximates  a PN triangle
 * surface.
 *
 */
 ...
 * approximates a PN triangle surface.
 *
 */
class PPSfromPNT : public PPS<dcel::Surface<VertexAttribute, FaceAttribute, int,
                                            HalfedgeAttribute>> {
public:
  /**
   * \using Surface
  ...
  /**
   * \fn PPSfromPNT( Surface* mesh )
   *
   * \brief Creates an instance of this PPSfromPNT class.
   *
   ...
   *
   * \brief  Computes a point  on the  PN triangle  patch associated
   * with a given face of the PPS underlying triangle mesh.
   *
...
public:
  ...
  /**
   * \fn Vertex* get_dst( Halfedge* h ) const
   *
  ...
  /**
   * \fn Edge* get_edge( Halfedge* h ) const
   *
...
private:
  /**
   * \fn void build_pnt_surface()
   *
   * \brief Computes a PN triangle  surface patch for each face of a
   * given triangle mesh, and then assigns the patch to the face.
   *
   */
  void build_pnt_surface();

  /**
   * \fn void compute_vertex_normal_vector( Vertex* vertex , double& x , double&
   * y , double& z )
   *
   * \brief Computes a unit vector at a given surface mesh vertex.
   *
   * \param vertex A pointer to a surface mesh vertex.
   * \param x A reference to the first component of the normal vector.
   * \param y A reference to the second component of the normal vector.
   * \param z A reference to the third component of the normal vector.
   */
  void compute_vertex_normal_vector(Vertex *vertex, double &x, double &y,
                                    double &z);

  /**
   * \fn void compute_face_normal_vector( Face* face , double& x , double& y ,
   * double& z )
   *
   * \brief Computes the unit vector of a given surface mesh face.
   *
   * \param face A pointer to a surface mesh face.
   * \param x A reference to the first component of the normal vector.
   * \param y A reference to the second component of the normal vector.
   * \param z A reference to the third component of the normal vector.
   */
